# this_file: src/vexy_dex/orchestrator.py
...
def run_strategy(
    ...
    strat = Strategy(name=exporter.name, exporter=exporter.name)
    try:
...
def build(source: Source, settings: Settings) -> list[DeckResult]:
    ...
    normalized = importer.transform(page, plan)
    ...
    results = [run_strategy(normalized, plan, ex, settings) for ex in exporters]
    write_sidecar(
