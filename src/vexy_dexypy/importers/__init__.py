# this_file: src/vexy_dexypy/importers/__init__.py
...
def discover_importers() -> dict[str, Importer]:
    ...
    try:
        eps = entry_points(group="vexy_dexypy.importers")
    except TypeError:  # pragma: no cover
        eps = entry_points().get("vexy_dexypy.importers", [])  # type: ignore[attr-defined]
    for ep in eps:
        try:
            inst = ep.load()()
            out[inst.name] = inst
