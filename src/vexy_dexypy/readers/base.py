# this_file: src/vexy_dexypy/readers/base.py
...
def discover_readers() -> list[Reader]:
    """Load reader plugins registered under the `vexy_dexypy.readers` group."""
    found: list[Reader] = []
    try:
        eps = entry_points(group="vexy_dexypy.readers")
    except TypeError:  # pragma: no cover - very old importlib API
        eps = entry_points().get("vexy_dexypy.readers", [])  # type: ignore[attr-defined]
    for ep in eps:
        try:
